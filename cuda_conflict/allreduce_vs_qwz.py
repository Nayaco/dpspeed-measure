...
def run_dual(rank, size):
    ...
    quant_size = 1024 * 1024 * 1024 * 2 # 4GB
    matmul_scale = (8192, 8192) # 4GB
    tensor_0 = torch.rand(comm_size, dtype=torch.float16, device=device)
    tensor_1 = torch.rand(quant_size, dtype=torch.float16, device=device)
    ...
    # warm up
    for i in range(3000):
        _ = quantizer_module.swizzle_quant(tensor_1, 1, 4, quantizer_module.Symmetric, 1, 1, 1)
    ...
    for i in range(30000):
        intra_quant_int4, intra_q_scales = quantizer_module.swizzle_quant(tensor_1, 1, 4, quantizer_module.Symmetric, 1, 1, 1)
    worker.wait()
...
def run_single(rank, size):
    ...
    device = torch.device('cuda:%d'%rank)
    comm_size = 1024 * 1024 * 512 # 1GB
    quant_size = 1024 * 1024 * 1024 * 2 # 1GB
    matmul_scale = (8192, 8192) # 4GB
    tensor_0 = torch.rand(comm_size, dtype=torch.float16, device=device)
    tensor_1 = torch.rand(quant_size, dtype=torch.float16, device=device)
    # warm up
    ...
    torch.cuda.synchronize(device=device)
    print('Rank ', rank, 'single quant in ', (end-start)*1000.0)
    # print('Rank ', rank, 'single quant in ', single_event_1_0.elapsed_time(single_event_1_1), 'ms')
    tensor_0 = tensor_0 + tensor_0
